...
def single_point_crossover(a: Genome, b: Genome) -> Tuple[Genome, Genome]:
    if(len(a) != len(b)):
        raise ValueError("Signe_point_crossover tried to compare 2 gen with differents length")
    length = len(a)
    if(length < 2):
        return a, b
...
def mutation(genome: Genome, num: int = 1, probability: float = 0.5) -> Genome :
    for _ in range(num):
        ...
        genome[index] = genome[index] if random() > probability else abs(genome[genome[index] -1])
        return genome
...
def run_evolution(
    ...
    for i in range(generation_limit):
        population = sorted(
            population,
            key= lambda genome: fitness_func(),
            reverse = True
        )
